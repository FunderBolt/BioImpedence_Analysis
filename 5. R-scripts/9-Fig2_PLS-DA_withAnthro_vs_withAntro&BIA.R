...
### log all variables
X<-log(X+10)
outcome<-as.factor(idata$group_kwash)
...
p2<-ggplot(aes(x=x, y=y), data=vars.in_anthro)+
    ...
    scale_color_manual(values=c("black","salmon","grey60"))+
    scale_fill_manual(values=alpha(c("#01FDF6","salmon","grey60"), 0.5))+
    ...
    stat_mean(aes(group = group),size=2, shape=8, color= c("black","salmon","grey30"), na.rm = TRUE, show.legend = NA,
              inherit.aes = TRUE)
p2
...
ggsave(file="Fig3_PLSDA_anthroOnly_vs_withBIVA_byGroups.svg", plot=p_all, width=10, height=10)
ggsave(file="Fig3_PLSDA_anthroOnly_vs_withBIVA_byGroups.pdf", plot=p_all, width=10, height=10)
...
### data with only antrho
X<-as.matrix(idata[!idata$group_kwash=="kwash",c("haz_adm","waz_adm","whz_adm","muac")])
### log all variables
X<-log(X+10)
outcome<-as.factor(idata$group_kwash[!idata$group_kwash=="kwash"])
### plsda between groups 
out.plsDA<-plsda(X,outcome, ncomp=2, scale=TRUE, mode="regression")

auc_MvsCP<-auroc(out.plsDA,roc.comp=1)
auc_MvsCP_data<-auc_MvsCP$graph.Comp1$data


#####################################################################
### data with antrho and BIA
X<-as.matrix(idata[!idata$group_kwash=="kwash",c("haz_adm","waz_adm","whz_adm","muac", "R.H_1","Xc.H_1","mean_ph_degrees_1")])
### log all variables
X<-log(X+10)
outcome<-as.factor(idata$group_kwash[!idata$group_kwash=="kwash"])
### plsda between groups
...
auc_MvsCP_BIA<-auroc(out.plsDA,roc.comp=1)
auc_MvsCP_BIA_data<-auc_MvsCP_BIA$graph.Comp1$data



auc_sW_CP<-ggplot()+
    geom_line(aes(x=Specificity, y=Sensitivity), color="grey10",size = 1.25, alpha=0.75, data=auc_MvsCP_data)+
    geom_line(aes(x=Specificity, y=Sensitivity),color="grey60", size = 1.25, alpha=0.75, data=auc_MvsCP_BIA_data)+
...
### data with only antrho
X<-as.matrix(idata[!idata$group_kwash=="maras",c("haz_adm","waz_adm","whz_adm","muac")])
### log all variables
X<-log(X+10)
outcome<-as.factor(idata$group_kwash[!idata$group_kwash=="maras"])
### plsda between groups 
out.plsDA<-plsda(X,outcome, ncomp=2, scale=TRUE, mode="regression")

auc_KvsCP<-auroc(out.plsDA,roc.comp=1)
auc_KvsCP_data<-auc_KvsCP$graph.Comp1$data


#####################################################################
### data with antrho and BIA
X<-as.matrix(idata[!idata$group_kwash=="maras",c("haz_adm","waz_adm","whz_adm","muac", "R.H_1","Xc.H_1","mean_ph_degrees_1")])
### log all variables
X<-log(X+10)
outcome<-as.factor(idata$group_kwash[!idata$group_kwash=="maras"])
### plsda between groups
...
auc_KvsCP_BIA<-auroc(out.plsDA,roc.comp=1)
auc_KvsCP_BIA_data<-auc_KvsCP_BIA$graph.Comp1$data



auc_E_CP<- ggplot()+
    geom_line(aes(x=Specificity, y=Sensitivity), color="grey10",size = 1.25, alpha=0.75, data=auc_KvsCP_data)+
    geom_line(aes(x=Specificity, y=Sensitivity),color="grey60", size = 1.25, alpha=0.75, data=auc_KvsCP_BIA_data)+
    geom_segment(aes(xend = 100, yend = 100, x = 0, y = 0), size = 0.75, colour = "grey60", linetype="dashed")+
    labs(title= "E vs. CP", x = "100 - Specificity (%)", 
         y = "Sensitivity (%)")+
    theme_bw()+
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
          title = element_text(size = 16),
...
### data with only antrho
X<-as.matrix(idata[!idata$group_kwash=="CP",c("haz_adm","waz_adm","whz_adm","muac")])
### log all variables
X<-log(X+10)
outcome<-as.factor(idata$group_kwash[!idata$group_kwash=="CP"])
### plsda between groups 
out.plsDA<-plsda(X,outcome, ncomp=2, scale=TRUE, mode="regression")

auc_KvsM<-auroc(out.plsDA,roc.comp=1)
auc_KvsM_data<-auc_KvsCP$graph.Comp1$data


#####################################################################
### data with antrho and BIA
X<-as.matrix(idata[!idata$group_kwash=="CP",c("haz_adm","waz_adm","whz_adm","muac", "R.H_1","Xc.H_1","mean_ph_degrees_1")])
### log all variables
X<-log(X+10)
outcome<-as.factor(idata$group_kwash[!idata$group_kwash=="CP"])
### plsda between groups
...
auc_KvsM_BIA<-auroc(out.plsDA,roc.comp=1)
auc_KvsM_BIA_data<-auc_KvsM_BIA$graph.Comp1$data



auc_E_K<- ggplot()+
    geom_line(aes(x=Specificity, y=Sensitivity), color="grey10",size = 1.25, alpha=0.75, data=auc_KvsM_data)+
    geom_line(aes(x=Specificity, y=Sensitivity),color="grey60", size = 1.25, alpha=0.75, data=auc_KvsM_BIA_data)+
    geom_segment(aes(xend = 100, yend = 100, x = 0, y = 0), size = 0.75, colour = "grey60", linetype="dashed")+
    labs(title= "sW vs. E", x = "100 - Specificity (%)", 
         y = "Sensitivity (%)")+
    theme_bw()+
    theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
          title = element_text(size = 16),
...
p_all_auc<-ggarrange(auc_E_K,auc_sW_CP,auc_E_CP, ncol = 3, nrow =1, common.legend = FALSE) 
p_all_auc
